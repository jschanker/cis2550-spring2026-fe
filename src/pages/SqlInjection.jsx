...
const SqlInjection = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/sql-injection/login`,
        {
          method: "POST",
  ...
  const fetchProducts = async (catInput) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/sql-injection/products?category=${encodeURIComponent(catInput)}`,
      );
      const result = await res.json();
      const query = result.pop()?.query || "";
      if (res.ok) {
        setProducts(result);
        setError(null);
      } else {
        const err = query;
        setError(query.message || "SQL Execution Error");
      ...
      <div className="max-w-5xl mx-auto space-y-12">
        <header className="border-b border-gray-700 pb-4">
          ...
          <p className="text-gray-400">
            Target Database:{" "}
            <span className="text-orange-400 font-mono italic underline">
              Oracle 19c (Simulated)
            </span>
          </p>
        </header>
        ...
        {loginMsg && (
          <div className="modal-overlay">
            <div
              className={`modal-box ${loginMsg.includes("ORA-") ? "modal-error" : "modal-info"}`}
            >
              {/* Terminal Top Bar */}
              <div className="modal-header">
                <span className="system-label">
                  {loginMsg.includes("ORA-")
              ...
              <div className="modal-body">
                <div className="message-wrapper">
                  <span className="status-icon">
                    {loginMsg.includes("ORA-") ? "✕" : "ℹ"}
                  </span>
                  <p className="status-text">{loginMsg}</p>
                </div>
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-700">
            <table className="w-full text-left border-collapse">
              <thead className="bg-gray-700 text-gray-300 uppercase text-xs tracking-wider">
                <tr>
                  {products.length > 0 ? (
                    getUniqueKeys(products).map((key) => (
                      <th
                        key={key}
